...
class Library:
    def __init__(self, id : int, meta: str, bs: str, ass_data: Dataset):
...
class Solve:
    ...
    def solve(self):
        ...
        for lib in self.lib_list:
            sl_list.append(self.solve_library(lib))

    def solve_library(self, time_limit, library : Library):
        time_limit = (time_limit - library.num_days) * library.num_ship
        library.book_table.sort(key=lambda x: self.data.score_table[x], reverse=True)
        books = []
        for i in range(0, time_limit):
            books.append(i)
        return SolutionLibrary(library.id, len(books), books)
...
def main(s: str):
    ...
    while (len(container) >= 2):
        ...
        if (not container[0].strip() == "" or not container[1].strip() == ""):
            lib_list.append(Library(iter, container[0], container[1], data))
            iter += 1
            container = container[2:]
        else:
            break

    #solve = Solve(data, lib_list)
    #solve.solve()
    ...
    for i in lib_list:
        ...
        for library in lib_list:
            if (library.num_days < lowest) and (i not in selected):
    ...
    for lnum in selected:
        ...
        output += str(lnum)
        output += ' '
        output += str(len(lib_list[lnum].book_table))
        output += "\n"
        file.write(output)
        output = ''
